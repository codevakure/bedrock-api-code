...
from api.models.models import FilterMode, GuardrailSettings
        ...
        for word, replacement in replacements.items():
            if settings.profanity_action == FilterMode.REMOVE:
                filtered_text = re.sub(
                    r"\b" + re.escape(word) + r"\b", "", filtered_text, flags=re.IGNORECASE
                )
            elif settings.profanity_action == FilterMode.MASK:
                filtered_text = re.sub(
                    r"\b" + re.escape(word) + r"\b", replacement, filtered_text, flags=re.IGNORECASE
                )
