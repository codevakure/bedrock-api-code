...
from api.models.models import GenerationSettings, QueryRequest
...
async def get_document(document_key: str):
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f'attachment; filename="{document_key}"'},
    )
...
async def health_check():
    return JSONResponse(
        {"status": "healthy", "service": "bedrock-api", "timestamp": datetime.now().isoformat()}
    )
