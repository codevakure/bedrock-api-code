...
from config.aws_config import bedrock_agent_runtime_client, bedrock_runtime, model_arn
...
class QueryService:
    ...
    async def stream_generate(
        ...
        try:
            ...
            metadata = {"page_numbers": [], "cost_metrics": None}
            ...
            if knowledge_base_id:
                ...
                try:
                    ...
                    if response and isinstance(response, dict):
                        ...
                        if "citations" in response:
                            ...
                            "retrieveAndGenerateResponse" in response
                            and "citations" in response["retrieveAndGenerateResponse"]
                        ):
                                    ...
                                    if "retrievedReferences" in citation:
                                        ...
                                        for reference in citation["retrievedReferences"]:
                                            if (
                                                ...
                                                and "metadata" in reference
                                            ):
                                                page_number = reference["metadata"].get(
                                                ...
                                                if page_number is not None:
                                                    try:
                                                        ...
                                                        print(
                                                            f"Could not convert page number to int: {page_number}"
                                                        )
                                        ...
                                        if (
                                            ...
                                            and page_numbers_for_citation
                                        ):
                                            citations_map[(span_start, span_end)] = sorted(
                                                page_numbers_for_citation
                                            )
                    ...
                    token_usage = KBCostMetrics.get_token_usage(
                        response, model_config.provider.value, model_config.model_id
                    )
                    ...
                    if token_usage["input_tokens"] <= 1 or token_usage["output_tokens"] <= 1:
                        ...
                        print(
                            f"DEBUG - Using estimated token counts: input={input_tokens}, output={output_tokens}"
                        )
                    ...
                    yield json.dumps(
                        {"error": f"Knowledge base query failed: {str(kb_error)}", "is_final": True}
                    ) + "\n"
                    return
                ...
                # Direct LLM query
                try:
                    request_body = KBUtils._prepare_request_body(
                        prompt, settings, current_model_arn
                    )
                    request_params = {
                    ...
                    # Try to get token usage from the response
                    token_usage = KBCostMetrics.get_token_usage(
                        response_body, model_config.provider.value, model_config.model_id
                    )
                    ...
                    yield json.dumps(
                        {"error": f"Direct LLM query failed: {str(llm_error)}", "is_final": True}
                    ) + "\n"
                    return
            ...
            for i in range(0, len(generated_text), chunk_size):
                ...
                response_chunk = {"chunk": chunk, "is_final": is_final}
