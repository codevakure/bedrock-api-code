...
class SchemaAnalyzerService:
    ...
    def save_schema(self, connection_id: str, schema_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully saved schema for connection {connection_id} version {version}"
            )
            return True
    ...
    def get_schema(
        self, connection_id: str, version: Optional[str] = None
        ...
        """
        logger.info(
            f"Getting schema for connection: {connection_id}, version: {version if version else 'latest'}"
        )
        try:
            if version:
    ...
    def delete_schema(self, connection_id: str, version: Optional[str] = None) -> bool:
        ...
        """
        logger.info(
            f"Deleting schema for connection: {connection_id}, version: {version if version else 'all'}"
        )
        try:
            if version:
                ...
                logger.info(
                    f"Successfully deleted schema for connection {connection_id} version {version}"
                )
                return True
            else:
                ...
                logger.info(
                    f"Found {len(items)} schema versions to delete for connection {connection_id}"
                )
                ...
                for i in range(0, len(items), batch_size):
                    ...
                    logger.info(
                        f"Deleting batch of {len(batch)} items for connection {connection_id}"
                    )
                ...
                logger.info(
                    f"Successfully deleted {len(items)} schema versions for connection {connection_id}"
                )
                return True
    ...
    def analyze_postgres_schema(
        ...
        logger.debug(
            f"Connection parameters: host={connection_params.get('host')}, "
        ...
        try:
            ...
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:
                ...
                for table_row in tables:
                    ...
                    try:
                        ...
                        table_description = (
                            table_desc_row["table_description"]
                            if table_desc_row and table_desc_row["table_description"]
                            else ""
                        )
                        logger.debug(
                            f"Table {table_name} description: {table_description[:50]}{'...' if len(table_description) > 50 else ''}"
                        )
                    except Exception as e:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error retrieving columns for table {table_name}: {e}", exc_info=True
                        )
                        columns_data = []
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error retrieving primary keys for table {table_name}: {e}",
                            exc_info=True,
                        )
                        primary_keys = []
                    ...
                    for col in columns_data:
                        ...
                        try:
                            ...
                            column_description = (
                                col_desc_row["description"]
                                if col_desc_row and "description" in col_desc_row
                                else ""
                            )
                        except Exception as e:
                            logger.warning(
                                f"Error retrieving description for column {col_name}: {e}"
                            )
                            column_description = ""
                        ...
                        if col["character_maximum_length"] is not None:
                            ...
                            col["numeric_precision"] is not None
            ...
            logger.debug(
                f"Schema has {len(schema_info['tables'])} tables and {len(schema_info['relationships'])} relationships"
            )
