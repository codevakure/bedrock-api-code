...
class KnowledgebaseMetricsService:
    ...
    def get_ai_usage_metrics() -> Dict:
        ...
        try:
            ...
            start_date = (datetime.utcnow() - relativedelta(months=12)).replace(day=1).strftime('%Y-%m-%d')
    ...
    def _is_ai_service(service: str) -> bool:
        ...
        ai_service_keywords = [
            'bedrock', 'claude', 'sagemaker', 'rekognition', 
            'comprehend', 'textract', 'transcribe', 'polly',
            'forecast', 'personalize', 'lex', 'deeplearning'
        ]
